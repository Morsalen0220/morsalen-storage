...
let lastLoginUsedCloudSession = false
const LOCAL_SESSION_KEY = "tglfs:local_session_v1"
...
const STORAGE_PEER_KEY = "tglfs:storage_peer"

function promptForPin(message: string): string | null {
    ...
    if (!isPinFormatValid(pin)) {
        alert(`PIN must be ${PIN_MIN_LEN}-${PIN_MAX_LEN} digits.`)
        return null
    }
    return pin
}
...
function loadLocalSession(): LocalSessionPayload | null {
    try {
        const raw = localStorage.getItem(LOCAL_SESSION_KEY)
        if (!raw) return null
        const parsed = JSON.parse(raw) as LocalSessionPayload
        if (!parsed?.phone || !parsed?.session) return null
        return parsed
    } catch {
        return null
    }
...
function saveLocalSession(payload: LocalSessionPayload) {
    try {
        localStorage.setItem(LOCAL_SESSION_KEY, JSON.stringify(payload))
    } catch (error) {
        console.warn("Failed to store local session", error)
    }
}

function normalizeStoragePeer(input: string | null | undefined): string {
    const value = (input ?? "").trim()
    return value.length > 0 ? value : "me"
}

function getStoragePeerSetting(): string {
    try {
        const raw = localStorage.getItem(STORAGE_PEER_KEY)
        return normalizeStoragePeer(raw)
    } catch {
        return "me"
    }
}

function setStoragePeerSetting(storagePeer: string) {
    try {
        localStorage.setItem(STORAGE_PEER_KEY, normalizeStoragePeer(storagePeer))
    } catch (error) {
        console.warn("Failed to store storage peer setting", error)
    }
}
...
async function maybeSetupCloudPinSession(client: any, phoneValue: string, storagePeer: string) {
    if (lastLoginUsedCloudSession) {
        return
    }

    try {
        const alreadyConfigured = await hasCloudSession()
        if (alreadyConfigured) {
            return
        }
        const shouldSetup = confirm(
            "Set up a PIN for next logins?\n\nNext time you can unlock from any device using only this PIN.",
        )
        if (!shouldSetup) {
            return
        }

        const pin = promptForPin("Create a PIN (4-12 digits):")
        if (!pin) {
            return
        }
        const confirmPin = promptForPin("Confirm your PIN:")
        if (!confirmPin) {
            return
        }
        if (pin !== confirmPin) {
        ...
        const Telegram = await getTelegramModule()
        const session = Telegram.exportSessionString(client)
        await saveCloudSession(pin, session, phoneValue, storagePeer)
        alert("PIN login is now enabled.")
    } catch (error) {
        console.error("Failed to set up cloud PIN session", error)
        alert("Could not save PIN session to Firestore.")
    }
}

async function logout() {
    const fullTelegramLogout = confirm(
        "Fully log out from Telegram for this session?\n\nChoose Cancel to only sign out from this device UI.",
    )
    try {
        if (fullTelegramLogout) {
    ...
    const shouldDeleteCloudSession = confirm(
        "Delete the saved cloud PIN session too?\n\nChoose OK to require full Telegram login next time everywhere.",
    )
    if (shouldDeleteCloudSession) {
        const pin = promptForPin("Enter PIN to confirm cloud session deletion:")
        if (pin) {
            try {
                await clearCloudSession(pin)
                alert("Cloud PIN session deleted.")
            } catch (error) {
                console.warn("Failed to clear cloud session", error)
                alert("Could not delete cloud session. PIN may be wrong.")
            }
        }
    }
...
async function finalizeLogin(client: any, config: any, phoneValue: string) {
    activeClient = client
    activeConfig = config
    const storagePeer = normalizeStoragePeer(config?.storagePeer)
    setStoragePeerSetting(storagePeer)
    try {
        const Telegram = await getTelegramModule()
        const session = Telegram.exportSessionString(client)
        if (session) {
            saveLocalSession({ phone: phoneValue, session, storagePeer })
        }
    } catch (error) {
        console.warn("Failed to cache local session", error)
    }
    ...
    await maybeSetupCloudPinSession(client, phoneValue, storagePeer)
    await maybeUploadSharedFiles()
}

async function startLogin(
    phoneNumber: string,
    options: { auto?: boolean; sessionString?: string; showLoginUi?: boolean; storagePeer?: string } = {},
): Promise<boolean> {
    ...
    const showLoginUi = options.showLoginUi !== false

    loginUi.setVisible(showLoginUi)
    if (showLoginUi) {
        loginUi.setStep("phone")
    ...
    lastLoginUsedCloudSession = !!options.sessionString
    if (showLoginUi) {
        loginUi.setStatus(
            options.sessionString
                ? "Unlocking your account with PIN..."
                : options.auto
    ...
    const storagePeer = normalizeStoragePeer(options.storagePeer ?? getStoragePeerSetting())
    const config = {
...
async function attemptPinLogin() {
    try {
        ...
        const pin = promptForPin("Enter your PIN to log in:")
        if (!pin) {
            return false
        }

        const payload = await loadCloudSession(pin)
        if (!payload.phone || !payload.session) {
            throw new Error("Stored PIN session is incomplete.")
        }
        const storagePeer = normalizeStoragePeer(payload.storagePeer ?? getStoragePeerSetting())
        saveLocalSession({ phone: payload.phone, session: payload.session, storagePeer })
        setStoragePeerSetting(storagePeer)
        return await startLogin(payload.phone, {
            auto: true,
            sessionString: payload.session,
            storagePeer,
        })
    } catch (error) {
        console.error("PIN login failed", error)
        alert("PIN login failed. Use phone login instead.")
        return false
    }
}
...
window.addEventListener("load", async () => {
    ...
    ;(window as any).setTglfsStoragePeer = (peer: string) => {
        const value = normalizeStoragePeer(peer)
        setStoragePeerSetting(value)
        alert(`Storage target set to ${value}. Reload to apply.`)
    }
    try {
        if (!localStorage.getItem(STORAGE_PEER_KEY)) {
            setStoragePeerSetting("me")
        }
    } catch (error) {
        console.warn("Failed to initialize storage target setting", error)
    }

    let started = false
    const localSession = loadLocalSession()
    if (localSession?.phone && localSession?.session) {
        const localLoginOk = await startLogin(localSession.phone, {
            auto: true,
            sessionString: localSession.session,
            showLoginUi: false,
            storagePeer: localSession.storagePeer,
        })
        if (localLoginOk) {
            started = true
        } else {
    ...
    if (!started) {
        loginUi.setVisible(true)
        const pinLoginStarted = await attemptPinLogin()
        if (!pinLoginStarted) {
            if (phone) {
