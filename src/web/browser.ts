...
async function loadFirstPage(client: any) {
    ...
    state.selected.clear()
    state.lastClickedIndex = null
    const items = await listFileCards(client, {
        query: state.query,
        limit: state.pageSize,
        storagePeer: activeStoragePeer,
    })
...
export async function initFileBrowser(client: any, config: Config.Config) {
    activeStoragePeer = (config.storagePeer ?? "me").trim() || "me"
    if (state.initialized) return
    state.initialized = true

    previewModal = new PreviewModal(client, activeStoragePeer)
    ...
    actionRename.addEventListener("click", async () => {
        ...
        await renameFileCard(client, selected.msgId, config.storagePeer ?? "me", selected.data, newName.trim())
    ...
    actionDelete.addEventListener("click", async () => {
        ...
        if (entries.length === 1) {
            ...
            if (!ok) return
            await deleteFileCard(client, selected.msgId, selected.data, activeStoragePeer)
            ...
            for (const entry of entries) {
                await deleteFileCard(client, entry.msgId, entry.data, activeStoragePeer)
            }
    ...
    actionSend.addEventListener("click", async () => {
        ...
        for (const entry of entries) {
            await sendFileCard(client, entry.data, recipient.trim(), config.storagePeer)
        }
