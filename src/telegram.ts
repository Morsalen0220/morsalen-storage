...
function getStoragePeer(config?: Config.Config): string {
    const raw = (config?.storagePeer ?? "me").trim()
    return raw.length > 0 ? raw : "me"
}
...
export async function fileUpload(client: TelegramClient, config: Config.Config, sharedFiles?: File[]) {
    ...
    const file = files[0]
    const storagePeer = getStoragePeer(config)
    ...
    try {
        ...
        const existingMsgs = await client.getMessages(storagePeer, {
        ...
        const fileCardMessage = await client.sendMessage(storagePeer, {
            message: `tglfs:file\n${JSON.stringify(fileCardData)}`,
        })

        const sendChunkToStorage = async (chunkFileUploaded: Api.InputFileBig, chunkNo: number) => {
            return await client.sendFile(storagePeer, {
                file: chunkFileUploaded,
                caption: `${displayName}\n${UFID}.chunk${chunkNo}`,
            })
        }
                    ...
                    while (encryptedDataBytesProcessed < encryptedData.length) {
                        ...
                        if (partBytesWritten === UPLOAD_PART_SIZE) {
                            ...
                            if (remainingChunkSpace <= UPLOAD_PART_SIZE) {
                                ...
                                const chunkMessage = await sendChunkToStorage(chunkFileUploaded, chunkIndex + 1)
...
export async function listFileCards(
    ...
    opts?: { query?: string; limit?: number; offsetId?: number; storagePeer?: string },
): Promise<Array<{ msgId: number; date: number; data: FileCardData }>> {
    const q = ("tglfs:file " + (opts?.query ?? "")).trim()
    const storagePeer = (opts?.storagePeer ?? "me").trim() || "me"
    const msgs = await client.getMessages(storagePeer, {
        search: q,
        limit: opts?.limit ?? 50,
        addOffset: 0,
        minId: 0,
...
export async function getFileCardByUfid(
    ...
    const peer = storagePeer.trim() || "me"
    const msgs = await client.getMessages(peer, { search: 'tglfs:file "ufid":"' + ufid.trim() + '"' })
...
export async function deleteFileCard(
    ...
    const peer = storagePeer.trim() || "me"
    await client.getMessages(peer, { ids: [msgId] })
...
export async function sendFileCard(
    ...
    for (let i = 0; i < data.chunks.length; i += BATCH_LIMIT) {
        for (let j = i; j < Math.min(i + BATCH_LIMIT, data.chunks.length); j++) {
            result = await client.invoke(
                new Api.messages.ForwardMessages({ fromPeer: peer, toPeer: recipient, id: [data.chunks[j]], silent: true }),
            )
            newChunkIds.push(result.updates[0].id)
        }
        await new Promise((resolve) => setTimeout(resolve, BATCH_DELAY))
...
export async function downloadFileCard(
    ...
    try {
        const chunkMsgs: Api.messages.Messages = await client.getMessages(storagePeer, { ids: data.chunks })
...
export async function init(config: Config.Config, authHandlers: AuthHandlers = {}): Promise<TelegramClient> {
    ...
    const stringSession = new StringSession(authHandlers.sessionString || "")
    // Connect.
